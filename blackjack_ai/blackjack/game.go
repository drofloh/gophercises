...
func New(opts Options) Game {
	...
	if opts.BlackackPayout == 0 {
		opts.BlackackPayout = 1.5
	}
	g.nDecks = opts.Hands
	g.nHands = opts.Hands
	g.blackjackPayout = opts.BlackackPayout
...
func MoveSplit(g *Game) error {
	cards := g.currentHand()
	if len(*cards) != 2 {
		return errors.New("you can only split  with 2 of the same card")
	}
